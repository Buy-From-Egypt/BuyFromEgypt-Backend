import { Controller, Get, Post, Patch, Body, Query, Logger } from '@nestjs/common';
...
export class ChatController {
  ...
  async getMessages(@Query() query: GetMessagesDto) {
    if (query.conversationId) {
      return this.chatService.getMessagesBetweenConversation(query.conversationId);
    }
  ...
  async createConversation(@Body('participantIds') ids: string[]) {
    return this.chatService.createConversation(ids);
  }
}
